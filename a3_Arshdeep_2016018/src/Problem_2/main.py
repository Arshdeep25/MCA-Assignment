...
from src.Problem_2.evaluation import evaluate_retrieval
from src.Problem_2.relevance_feedback import relevance_feedback, relevance_feedback_exp
...
def tf_idf(docs, queries, tokenizer):
    ...
    processed_docs = [d.lower().translate(str.maketrans(string.punctuation+string.digits, ' '.rjust(len(string.punctuation+string.digits)))) for d in docs]
...
if __name__ == '__main__':
    ...
    vec_docs, vec_queries, tfidf_model = tf_idf(docs, queries, tokenize_text)
    rf_sim_matrix = relevance_feedback_exp(vec_docs, vec_queries, sim_matrix, tfidf_model, queries)
    if args.verbose:
        for i in range(rf_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rf_sim_matrix[:, i])
            print('Query:', i+1, 'Top relevant 10 documents:', ranked_documents[:10] + 1)
